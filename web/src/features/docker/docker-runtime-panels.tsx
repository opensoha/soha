...
function dockerRuntimeWebSocketURL(projectId: string, path: 'logs/stream' | 'terminal', params: Record<string, string | number | undefined>, accessToken?: string | null) {
  const base = API_BASE_URL.startsWith('http')
    ? new URL(API_BASE_URL)
    : new URL(API_BASE_URL, window.location.origin)
  base.protocol = base.protocol === 'https:' ? 'wss:' : 'ws:'
  base.pathname = `${base.pathname.replace(/\/$/, '')}/docker/projects/${encodeURIComponent(projectId)}/runtime/${path}`
...
export function DockerProjectLogsPanel({
  ...
  const streamURL = useMemo(() => {
    ...
    return dockerRuntimeWebSocketURL(projectId, 'logs/stream', { serviceName, tailLines }, accessToken)
  }, [accessToken, projectId, serviceName, tailLines])
  ...
  useEffect(() => {
    ...
    socket.onclose = () => setConnectionState((current) => current === 'error' ? 'error' : 'closed')
    return () => {
      closeStream()
    }
...
export function DockerProjectTerminalPanel({
  ...
  const terminalURL = useMemo(() => {
    if (!projectId || !serviceName) return ''
    return dockerRuntimeWebSocketURL(projectId, 'terminal', { serviceName, shell }, accessToken)
  }, [accessToken, projectId, serviceName, shell])
