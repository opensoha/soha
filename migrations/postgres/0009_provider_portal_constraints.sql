...
UPDATE identity_applications a
SET
    provider_id = NULL,
    updated_by = 'system',
    updated_at = now()
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_identity_providers_id_application_type
    ON identity_providers (id, application_id, type);
...
UPDATE identity_provider_signing_keys k
SET
    active = FALSE,
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_identity_provider_signing_keys_one_active
    ON identity_provider_signing_keys (provider_id)
    WHERE active = TRUE;
...
BEGIN
    IF NOT EXISTS (
        ...
        ALTER TABLE identity_applications
            ...
            REFERENCES identity_providers (id, application_id, type);
    END IF;
END $$;
...
BEGIN
    IF NOT EXISTS (
        ...
        ALTER TABLE identity_oidc_clients
