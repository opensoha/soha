...
CREATE TABLE IF NOT EXISTS public.network_access_nas_bindings (
    id text PRIMARY KEY,
    ...
    disconnect_supported boolean NOT NULL DEFAULT false,
    created_at timestamptz NOT NULL DEFAULT now(),
    updated_at timestamptz NOT NULL DEFAULT now(),
    UNIQUE (tenant_id, workspace_id, nas_id)
);

CREATE TABLE IF NOT EXISTS public.network_access_site_profile_bindings (
    id text PRIMARY KEY,
    tenant_id text NOT NULL DEFAULT 'default',
    workspace_id text NOT NULL DEFAULT 'default',
    site_id text NOT NULL REFERENCES public.network_access_sites(id) ON DELETE RESTRICT,
    ...
    filter_id text,
    session_timeout_seconds integer NOT NULL CHECK (session_timeout_seconds BETWEEN 1 AND 86400),
    created_at timestamptz NOT NULL DEFAULT now(),
    updated_at timestamptz NOT NULL DEFAULT now(),
    UNIQUE (tenant_id, workspace_id, site_id, access_profile)
);
...
CREATE INDEX IF NOT EXISTS idx_network_access_nas_bindings_scope
    ON public.network_access_nas_bindings (tenant_id, workspace_id, site_id, runtime_id, status, nas_id);
CREATE INDEX IF NOT EXISTS idx_network_access_site_profile_bindings_scope
    ON public.network_access_site_profile_bindings (tenant_id, workspace_id, site_id, access_profile);
