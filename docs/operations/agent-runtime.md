...
kubecrux 的 agent 进程同时承担两类运行时职责：

- 远程集群 agent：为 `connectionMode=agent` 的 Kubernetes 集群提供资源读取和有限操作入口。
- 控制平面 runner：从 kubecrux 控制面领取 delivery、Docker 和 AI Agent Runtime 任务，通过回调写回状态和结果。

AI Agent Runtime 是 kubecrux 面向 AI 工作台、自动化策略和业务诊断场景的统一执行抽象。页面和业务模块只面向 kubecrux 的能力契约，不直接依赖 Hermes、OpenClaw 或任何具体 agent。
...
- `AgentSkillBinding`：kubecrux skill 到 Hermes skill、prompt template 或未来 provider skill 机制的映射。
- `AgentRun`：一次 durable agent 执行，包含 provider、capability、skills、toolset、scope、状态、回调 token、工具记录和输出工件。

Hermes Agent 是当前第一个外部 provider。它通过 kubecrux agent runner 领取 `ai_agent_runs`，执行 Hermes CLI 或 Hermes Agent 能力，然后把结果统一回写为 `AnalysisArtifact`。

`AgentRun` 会保存创建时的 `toolBindings` 和 `skillBindings` 快照。runner claim 拿到的是一次完整的 kubecrux capability contract：scope、toolset、预算、平台 playbook、provider skill 映射、MCP/internal tool 映射都会随任务一起下发。后续接 OpenClaw、internal agent 或其它 provider 时，应读取这份快照来实现 provider adapter，而不是在页面或业务模块里重新推导 Hermes/OpenClaw 私有参数。
...
`tool-call` 是外部 provider 调用 kubecrux 工具的受控网关。请求必须同时携带 runner bearer token 和当前 `AgentRun.callbackToken`，并且只能调用该 run 创建时快照到 `toolBindings` 的只读工具。控制面当前可执行日志、指标、链路、平台事件、交付发布、交付构建和告警查询，并把每次调用写入 `ToolExecution`，供最终 `AnalysisArtifact` 复用。Provider adapter 不应该绕过这个网关直接访问 kubecrux 数据源或凭据。

当前 runner 的 Hermes/CLI provider POC 会在执行 provider 命令前预取最多 3 个可预取只读工具结果，例如 `events.query`、`logs.query`、`metrics.query`、`traces.query`、`delivery.releases.list`、`delivery.builds.list`、`alerts.list`，并把结果作为 `prefetchedToolResults` 注入 provider prompt 和最终 output。这样即使 provider 还没有接入私有 tool-call 协议，也能消费 kubecrux 受控工具上下文。后续如果 Hermes 或其它 agent 使用 MCP client 方式接入，应在 runner adapter 层把同一个 `tool-call` 网关包装成 provider 可识别的 MCP/tool server，仍然不暴露 kubecrux 数据源凭据。

当前目录中还声明了 `delivery.execution_tasks.list`、`platform.resources.snapshot`、`docker.operations.list`、`docker.services.list`、`virtualization.operations.list`、`oncall.routes.resolve` 等能力绑定，它们是 Agent Runtime 的稳定 catalog contract。只有在对应 reader 或 adapter 接入 `executeAgentToolBindingOutput` 后才会成为可执行工具；在此之前 runner 不会把这些绑定加入默认预取集合，provider 直接调用时会收到显式 unsupported 错误。
...
3. `internal` provider 继续走 kubecrux 内置同步分析路径。
...
7. provider 如需查询日志、指标、链路、事件、交付发布/构建或告警，应由 runner adapter 通过 `agent-runs/tool-call` 调用 kubecrux 受控工具。
8. runner 通过 callback 回写 `running`、`completed` 或 `failed`。
9. 控制面把 provider 输出转换为 kubecrux `AnalysisArtifact`，继续沉淀证据、假设、建议、关系图、工具调用和数据源快照。

自动化策略链路：

1. kubecrux automation policy 仍是持续分析的主调度源。
...
5. Hermes cron 只作为实验性 provider 原生能力，不作为 kubecrux 平台主调度源。
...
- runner 会把 kubecrux run 输入、tool binding 快照、skill binding 快照转换为 prompt，并把 skill id 传给 Hermes。
...
- runner 回调中的 `analysisArtifacts` 优先作为最终工件；如果缺失，控制面会用输出摘要合成一个基础 `AnalysisArtifact`。
...
这种 provider executor 仍然只接收 kubecrux `AgentRun` 合约：prompt 内包含 scope、toolset、toolBindings、skillBindings 和 output schema；provider 原生 skill 参数来自 `AgentSkillBinding.providerSkillRef`。如果没有配置对应 provider executor，runner 会显式回调 failed，而不是把未知 provider 伪装成 Hermes 执行。
...
5. 保持 callback 输出为 kubecrux `AnalysisArtifact` 或可被合成为 `AnalysisArtifact` 的结构。

硬性边界：

- 页面、handler 和业务模块不得直接调用 Hermes。
- 预算、权限、菜单、审计、数据源脱敏和操作边界由 kubecrux 控制。
- Agent 只作为可插拔执行器，不能绕过 kubecrux 的 MCP adapter、scope、toolset、tool-call 网关和 analysis profile 合约。
- 高风险写操作不能自动挂到聊天链路，必须先落到对应业务模块的 durable operation 或 approval 流程。

## 远程集群 Agent 范围

同一个 `cmd/agent` 仍保留远程集群 agent 能力：
