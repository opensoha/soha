...
- `internal/agent/runner` 具备不依赖真实 Hermes 安装的 smoke 覆盖：fake control plane 可验证 claim -> running callback -> tool-call prefetch -> CLI provider -> completed callback -> `AnalysisArtifact` 的完整协议链路。真实环境仍应把 `hermes_command` 指向 Hermes CLI 做最后端到端验证。
