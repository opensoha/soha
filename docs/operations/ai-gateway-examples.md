...
This page gives copyable operating examples for `soha-cli` and the AI Gateway. The examples assume the backend is reachable at `http://localhost:8080`, the bootstrap admin user can manage Gateway objects, and `soha-cli` is available on the local `PATH`.
...
```bash
soha-cli governance status --profile gateway-admin --json
...
```bash
soha-cli mcp install --profile gateway-admin --command /usr/local/bin/soha-cli
```
...
For project-local or team-managed setups, keep the `soha-cli` profile in the user account that runs the editor. The MCP process reads `~/.soha/config.json` by default, or the path in `SOHA_CONFIG`.
...
```bash
soha-cli service-account create \
  --profile gateway-admin \
  --id ci-delivery \
  --name "CI Delivery" \
...
```bash
soha-cli service-account token-create \
  --profile gateway-admin \
...
soha-cli capabilities --profile ci --output names
```

If the CI job only needs MCP, point the client at:

```bash
soha-cli mcp start --profile ci --ai-client-id ci-github-actions --ai-client "GitHub Actions" --skill-id delivery-developer
```
...
- **Personal access tokens** maps to `personal_access_tokens`. The create form fields are `name`, `permissionKeys`, `scopes`, and `expiresAt`. The plaintext token is shown once after creation and is not stored in browser state.
- **Service accounts** maps to `service_accounts` and `service_account_tokens`. Service account fields are optional `id`, `name`, `description`, `status`, `roleIds`, `teamIds`, and `scopeGrantIds`. Service token fields are `name`, `permissionKeys`, `scopes`, and `expiresAt`; the token table shows id, owner service account, prefix, permissions, scopes, expiration, last-used, and revocation state, and revocation accepts a token id.
...
```bash
soha-cli tool call delivery.applications.detail \
  --profile gateway-admin \
  --input-json '{"applicationId":"app-1"}'

soha-cli tool call delivery.application_services.list \
  --profile gateway-admin \
  --input-json '{"applicationId":"app-1"}'
...
soha-cli tool call delivery.release_context.diff \
  --profile gateway-admin \
...
```bash
soha-cli tool call delivery.actions.trigger \
  --profile gateway-admin \
...
```bash
soha-cli tool call delivery.rollback.context \
  --profile gateway-admin \
...
```bash
soha-cli tool call k8s.deployments.rollout_status \
  --profile gateway-admin \
...
```bash
soha-cli tool call k8s.pods.describe \
  --profile gateway-admin \
...
```bash
soha-cli tool call diagnosis.release_failure.analyze \
  --profile gateway-admin \
...
```bash
soha-cli diagnose --profile gateway-admin --tool diagnosis.release_failure.analyze
soha-cli capabilities --profile gateway-admin --json
```
...
```bash
soha-cli audit list --profile gateway-admin --ai-client-id codex-local --limit 20
soha-cli governance status --profile gateway-admin --window-hours 24
```
