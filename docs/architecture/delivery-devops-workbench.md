...
| 层级 | 对象 | 说明 |
| --- | --- | --- |
| 主数据环境 | `environments` | 定义 `dev`、`test`、`sit`、`uat`、`pre`、`prod` 等环境及排序、生产标识、审批要求 |
| 应用环境绑定 | `application_environments` | 某个应用在某个环境中的交付策略、工作流模板、构建策略、发布策略和目标集合 |
...
- 应用名、业务线、负责人、服务数、环境数、最近 Bundle、最近执行状态。
- 开发环境状态、测试环境状态、生产版本。
- 快捷入口：详情、构建、环境矩阵、交付物。
