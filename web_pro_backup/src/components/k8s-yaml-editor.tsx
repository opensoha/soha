...
export function K8sYamlEditor({
  ...
  const yamlOptions = useMemo<MonacoYamlOptions>(() => ({
        ...
        <Editor
          ...
          beforeMount={(monaco) => {
            ...
            if (!yamlHandleRef.current) {
              yamlHandleRef.current = configureMonacoYaml(monaco, yamlOptions)
              return
            }
            void yamlHandleRef.current.update(yamlOptions)
          }}
