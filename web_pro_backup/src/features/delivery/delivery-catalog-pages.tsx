...
import {
  deliveryApplicationEnvironmentsQueryKey,
  deliveryApplicationEnvironmentsQueryOptions,
  deliveryApplicationsQueryOptions,
  deliveryBuildsQueryOptions,
  deliveryBusinessLinesQueryKey,
  deliveryBusinessLinesQueryOptions,
  deliveryEnvironmentsQueryKey,
  deliveryEnvironmentsQueryOptions,
  deliveryReleasesQueryKey,
  deliveryReleasesQueryOptions,
  deliveryWorkflowTemplatesQueryKey,
  deliveryWorkflowTemplatesQueryOptions,
...
export function BusinessLinesPage({ embedded = false }: EmbeddedPageProps = {}) {
  const { t } = useI18n()
  ...
  const { data, isLoading } = useQuery(deliveryBusinessLinesQueryOptions())

  const createMutation = useMutation({
    mutationFn: (values: Record<string, unknown>) => api.post('/business-lines', values),
    onSuccess: () => {
      message.success('业务线创建成功')
      queryClient.invalidateQueries({ queryKey: deliveryBusinessLinesQueryKey })
  ...
  return (
    <div className="kc-page">
      {!embedded ? <PageHeader title={t('page.delivery.businessLines.title', 'Business Lines')} description="维护交付主数据中的业务线范围、负责人和排序。" /> : null}
      <AdminTable
...
export function DeliveryEnvironmentsPage({ embedded = false }: EmbeddedPageProps = {}) {
  const { t } = useI18n()
  const { message } = App.useApp()
  const queryClient = useQueryClient()
  const permissionSnapshotQuery = usePermissionSnapshot()
  ...
  const { data, isLoading } = useQuery(deliveryEnvironmentsQueryOptions())

  const createMutation = useMutation({
    mutationFn: (values: Record<string, unknown>) => api.post('/delivery-environments', values),
    onSuccess: () => {
      message.success('环境创建成功')
      queryClient.invalidateQueries({ queryKey: deliveryEnvironmentsQueryKey })
      setModalVisible(false)
    },
    onError: (err: Error) => message.error(err.message),
  })
  ...
  return (
    <div className="kc-page">
      {!embedded ? <PageHeader title={t('page.delivery.environments.title', 'Environments')} description="维护环境定义、分层顺序和审批约束。" /> : null}
      <AdminTable
...
export function ApplicationEnvironmentsPage({ embedded = false }: EmbeddedPageProps = {}) {
  const { t } = useI18n()
  const { message } = App.useApp()
  const queryClient = useQueryClient()
  const permissionSnapshotQuery = usePermissionSnapshot()
  ...
  const bindingsQuery = useQuery(deliveryApplicationEnvironmentsQueryOptions())
  const appsQuery = useQuery(deliveryApplicationsQueryOptions<ApplicationOption>())
  const environmentsQuery = useQuery(deliveryEnvironmentsQueryOptions())
  const workflowTemplatesQuery = useQuery(deliveryWorkflowTemplatesQueryOptions())
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: deliveryApplicationEnvironmentsQueryKey })
      setModalVisible(false)
    },
    onError: (err: Error) => message.error(err.message),
  })
  ...
  return (
    <div className="kc-page">
      {!embedded ? <PageHeader title={t('page.delivery.bindings.title', 'Application Environment Bindings')} description="维护应用与环境的绑定、发布模板和目标清单。" /> : null}
      <AdminTable
...
export function ReleaseBoardPage({ embedded = false }: EmbeddedPageProps = {}) {
  ...
  const applicationsQuery = useQuery(deliveryApplicationsQueryOptions<ApplicationOption>())
  const environmentsQuery = useQuery(deliveryEnvironmentsQueryOptions())
  const bindingsQuery = useQuery(deliveryApplicationEnvironmentsQueryOptions())
  const buildsQuery = useQuery(deliveryBuildsQueryOptions<BuildRecord>())
  const workflowsQuery = useQuery(deliveryWorkflowsQueryOptions<WorkflowRecord>())
  const releasesQuery = useQuery(deliveryReleasesQueryOptions<ReleaseRecord>())
  ...
  return (
    <div className="kc-page">
      {!embedded ? <PageHeader title={t('page.releaseBoard.title', 'Release Board')} description={t('page.releaseBoard.desc', 'Inspect target bindings by application and environment as the entry point for workflow and deployment linkage.')} /> : null}
      <Card>
...
export function WorkflowTemplatesPage({ embedded = false }: EmbeddedPageProps = {}) {
  const { t, localeCode } = useI18n()
  const { message } = App.useApp()
  const queryClient = useQueryClient()
  const permissionSnapshotQuery = usePermissionSnapshot()
  ...
  const { data, isLoading } = useQuery(deliveryWorkflowTemplatesQueryOptions())
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: deliveryWorkflowTemplatesQueryKey })
      setModalVisible(false)
    },
    onError: (err: Error) => message.error(err.message),
  })
