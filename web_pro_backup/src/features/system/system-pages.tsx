...
export function OnlineUsersPage({ embedded = false }: EmbeddedPageProps = {}) {
  ...
  const { data, isLoading } = useQuery(onlineUsersQueryOptions())

  const revokeMutation = useMutation({
    mutationFn: (sessionId: string) => api.post(`/auth/sessions/${sessionId}/revoke`),
    onSuccess: (_result, sessionId) => {
      void message.success('用户会话已下线')
      void queryClient.invalidateQueries({ queryKey: onlineUsersQueryKey })
...
export function AnnouncementsPage({ embedded = false }: EmbeddedPageProps = {}) {
  const queryClient = useQueryClient()
  const permissionSnapshotQuery = usePermissionSnapshot()
  ...
  const { data, isLoading } = useQuery<ApiResponse<Announcement[]>>(announcementsQueryOptions())
...
export function MenusPage({ embedded = false }: EmbeddedPageProps = {}) {
  const queryClient = useQueryClient()
  const permissionSnapshotQuery = usePermissionSnapshot()
  ...
  const { data, isLoading } = useQuery<ApiResponse<MenuItem[]>>(menusQueryOptions())
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      void queryClient.invalidateQueries({ queryKey: menusQueryKey })
      setModalVisible(false)
    },
    onError: (err: Error) => void message.error(err.message),
  })
...
export function AuditLogsPage({ embedded = false }: EmbeddedPageProps = {}) {
  ...
  return (
    <div className="kc-page">
      {!embedded ? <PageHeader title="审计日志" description="查看关键操作的审计记录、资源对象和执行结果。" /> : null}
...
export function OperationLogsPage({ embedded = false }: EmbeddedPageProps = {}) {
  const { data, isLoading } = useQuery({
  ...
  return (
    <div className="kc-page">
      {!embedded ? <PageHeader title="操作日志" description="查看系统操作流水、目标对象与处理状态。" /> : null}
      <AdminTable columns={columns} dataSource={data?.data ?? []} rowKey="id" loading={isLoading} pageSize={50} />
    </div>
  )
}
