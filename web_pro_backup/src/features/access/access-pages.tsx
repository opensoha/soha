...
interface AccessCrudConfig<T extends { id: string }> {
  path: string
  listQueryOptions: () => {
    queryKey: QueryKey
...
function useCRUD<T extends { id: string }>(
  ...
  const invalidateAll = () => {
    queryClient.invalidateQueries({ queryKey: config.listQueryOptions().queryKey })
    config.invalidateKeys?.forEach((key) => queryClient.invalidateQueries({ queryKey: key }))
  }

  const query = useQuery(config.listQueryOptions())

  const createMutation = useMutation({
    mutationFn: (values: Record<string, unknown>) => api.post(`/${config.path}`, values),
    onSuccess: () => {
...
function ScopeGrantManager({
  ...
  const grantsQuery = useQuery({ ...accessScopeGrantsQueryOptions(), enabled: visible })
  const businessLinesQuery = useQuery<ApiResponse<BusinessLine[]>>({ ...deliveryBusinessLinesQueryOptions(), enabled: visible })
  const environmentsQuery = useQuery<ApiResponse<DeliveryEnvironment[]>>({ ...deliveryEnvironmentsQueryOptions(), enabled: visible })
  const applicationsQuery = useQuery<ApiResponse<Array<{ id: string; name: string }>>>({ ...deliveryApplicationsQueryOptions(), enabled: visible })
...
export function AccessUsersPage() {
  ...
  const usersQuery = useQuery<ApiResponse<AccessUser[]>>(accessUsersQueryOptions())
  const rolesQuery = useQuery<ApiResponse<AccessRole[]>>(accessRolesQueryOptions())
  const teamsQuery = useQuery<ApiResponse<AccessTeam[]>>(accessTeamsQueryOptions())
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: accessUsersQueryKey })
      setModalVisible(false)
    },
    onError: (err: Error) => message.error(err.message),
  })
...
export function AccessRolesPage() {
  ...
  const [form] = Form.useForm<Record<string, unknown>>()
  const crud = useCRUD<AccessRole>({
    path: 'access/roles',
    listQueryOptions: accessRolesQueryOptions,
    invalidateKeys: [accessUsersQueryKey],
  })
...
export function AccessTeamsPage() {
  ...
  const canManageScopeGrants = hasPermission(snapshot, 'access.scope-grants.manage')
  const [form] = Form.useForm<Record<string, unknown>>()
  const crud = useCRUD<AccessTeam>({
    path: 'access/teams',
    listQueryOptions: accessTeamsQueryOptions,
    invalidateKeys: [accessUsersQueryKey, accessScopeGrantsQueryKey],
  })
...
export function AccessPoliciesPage() {
  ...
  const [form] = Form.useForm<Record<string, unknown>>()
  const crud = useCRUD<AccessPolicy>({
    path: 'access/policies',
    listQueryOptions: accessPoliciesQueryOptions,
  })
