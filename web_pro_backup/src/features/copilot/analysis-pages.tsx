...
import {
  copilotInspectionTasksQueryKey,
  copilotInspectionTasksQueryOptions,
  copilotRootCauseRunsQueryKey,
  copilotRootCauseRunsQueryOptions,
} from '@/features/copilot/queries'
import { monitoringSummaryQueryOptions, type MonitoringSummary } from '@/features/observability/queries'
...
function buildPerformanceSignals(summary?: MonitoringSummary, clusters: Cluster[] = [], localeCode: 'zh_CN' | 'en_US' = 'zh_CN') {
...
export function RootCauseAnalysisPage({ embedded = false }: EmbeddedPageProps = {}) {
  ...
  const runsQuery = useQuery<ApiResponse<RootCauseRun[]>>(copilotRootCauseRunsQueryOptions())
  ...
  const createRunMutation = useMutation({
    ...
    onSuccess: (response) => {
      ...
      queryClient.invalidateQueries({ queryKey: copilotRootCauseRunsQueryKey })
...
export function PerformanceAnalysisPage({ embedded = false }: EmbeddedPageProps = {}) {
  const { localeCode } = useI18n()
...
export function InspectionCenterPage({ embedded = false }: EmbeddedPageProps = {}) {
  const { localeCode } = useI18n()
  const queryClient = useQueryClient()
  const permissionSnapshotQuery = usePermissionSnapshot()
  const [form] = Form.useForm()
  ...
  const tasksQuery = useQuery<ApiResponse<InspectionTask[]>>(copilotInspectionTasksQueryOptions())
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: copilotInspectionTasksQueryKey })
