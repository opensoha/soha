...
export function ConfigMapDetailPage({ embedded = false }: { embedded?: boolean }) {
  const { t, localeCode } = useI18n()
  ...
  return (
    <div className="kc-page">
      ...
      {embedded ? (
        <Card className="kc-detail-card" title={`ConfigMap: ${name}`}>
          <Text type="secondary">
            {localeCode === 'zh_CN' ? '查看 ConfigMap 的键值数据与 YAML。' : 'Inspect ConfigMap data entries and YAML.'}
          </Text>
        </Card>
      ) : null}
      <Tabs
...
export function SecretDetailPage({ embedded = false }: { embedded?: boolean }) {
  const { t, localeCode } = useI18n()
  const navigate = useNavigate()
  const params = useParams()
  ...
  return (
    <div className="kc-page">
      ...
      {embedded ? (
        <Card className="kc-detail-card" title={`Secret: ${name}`}>
          <Text type="secondary">
            {localeCode === 'zh_CN' ? '查看 Secret 的数据(默认 base64)与 YAML。敏感信息仅限授权查看。' : 'Inspect Secret data (base64 by default) and YAML.'}
          </Text>
        </Card>
      ) : null}
      <Tabs
