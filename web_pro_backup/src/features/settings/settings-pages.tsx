...
import {
  copilotAnalysisProfilesQueryKey,
  copilotAnalysisProfilesQueryOptions,
  copilotAutomationPoliciesQueryKey,
  copilotAutomationPoliciesQueryOptions,
  copilotDataSourceCapabilitiesQueryOptions,
  copilotDataSourcesQueryKey,
  copilotDataSourcesQueryOptions,
  settingsAIQueryKey,
  settingsAIQueryOptions,
  settingsBrandingQueryKey,
  settingsBrandingQueryOptions,
  settingsIdentityQueryKey,
  settingsIdentityQueryOptions,
  settingsMonitoringQueryKey,
  settingsMonitoringQueryOptions,
  type OIDCSettings,
...
export function BrandingSettingsPage({ embedded = false }: SettingsPageProps = {}) {
  ...
  const { data, isLoading } = useQuery(settingsBrandingQueryOptions())

  const saveMutation = useMutation({
    mutationFn: (values: Record<string, unknown>) => api.put('/settings/branding', values),
    onSuccess: () => {
      void message.success('品牌设置已保存')
      void queryClient.invalidateQueries({ queryKey: settingsBrandingQueryKey })
    },
...
export function IdentitySettingsPage({ embedded = false }: SettingsPageProps = {}) {
  ...
  const { data, isLoading } = useQuery(settingsIdentityQueryOptions())

  const saveMutation = useMutation({
    mutationFn: (values: Record<string, unknown>) => api.put('/settings/identity/oidc', values),
    onSuccess: () => {
      void message.success('身份设置已保存')
      void queryClient.invalidateQueries({ queryKey: settingsIdentityQueryKey })
    },
    onError: (err: Error) => void message.error(err.message),
  })
...
export function MonitoringSettingsPage({ embedded = false }: SettingsPageProps = {}) {
  const queryClient = useQueryClient()
  const permissionSnapshotQuery = usePermissionSnapshot()
  ...
  const { data, isLoading } = useQuery(settingsMonitoringQueryOptions())

  const saveMutation = useMutation({
    mutationFn: (values: Record<string, unknown>) => api.put('/settings/monitoring/prometheus', values),
    onSuccess: () => {
      void message.success('监控设置已保存')
      void queryClient.invalidateQueries({ queryKey: settingsMonitoringQueryKey })
    },
    onError: (err: Error) => void message.error(err.message),
  })
  ...
  return (
    <div className="kc-page">
      {!embedded ? <PageHeader title="监控设置" description="配置 Prometheus 地址、默认查询范围和访问凭证。" /> : null}
      <SettingsCard>
        <Form
          {...DEFAULT_FORM_LAYOUT}
          onFinish={(values) => {
...
export function AISettingsPage({ embedded = false }: SettingsPageProps = {}) {
  ...
  const { data, isLoading } = useQuery(settingsAIQueryOptions())
  const dataSourcesQuery = useQuery(copilotDataSourcesQueryOptions<DataSource>())
  const profilesQuery = useQuery(copilotAnalysisProfilesQueryOptions<AnalysisProfile>())
  const policiesQuery = useQuery(copilotAutomationPoliciesQueryOptions<AutomationPolicy>())
  const capabilitiesQuery = useQuery(copilotDataSourceCapabilitiesQueryOptions())

  const saveMutation = useMutation({
    mutationFn: (values: Record<string, unknown>) => api.put('/settings/ai/provider', values),
    onSuccess: () => {
      void message.success('AI 设置已保存')
      void queryClient.invalidateQueries({ queryKey: settingsAIQueryKey })
    },
    onError: (err: Error) => void message.error(err.message),
  })
  const dataSourceMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      void queryClient.invalidateQueries({ queryKey: copilotDataSourcesQueryKey })
  ...
  const profileMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      void queryClient.invalidateQueries({ queryKey: copilotAnalysisProfilesQueryKey })
  ...
  const policyMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      void queryClient.invalidateQueries({ queryKey: copilotAutomationPoliciesQueryKey })
...
export function SettingsCenterPage({ embedded = false }: SettingsPageProps = {}) {
  ...
  if (permissionSnapshotQuery.isLoading) {
    return (
      <div className="kc-page">
        {!embedded ? <PageHeader title="设置中心" description="集中配置身份与 AI 能力。" /> : null}
