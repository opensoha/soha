...
func (d *Direct) ReviewSubjectAccess(ctx context.Context, clusterID string, input domainresource.SubjectAccessReviewInput) (domainresource.SubjectAccessReviewResult, error) {
	...
	for _, check := range input.Checks {
		review, err := bundle.Typed.AuthorizationV1().SubjectAccessReviews().Create(queryCtx, &authorizationv1.SubjectAccessReview{
