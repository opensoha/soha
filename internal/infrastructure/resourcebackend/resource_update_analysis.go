...
func analyzeResourceUpdate(live, desired *unstructured.Unstructured) domainresource.ResourceUpdateAnalysis {
	analysis := domainresource.ResourceUpdateAnalysis{
		FieldManager: resourceEditFieldManager, ChangedFields: []string{},
		Owners: []domainresource.ManagedFieldOwner{}, Conflicts: []domainresource.FieldConflict{},
	}
	...
	for _, entry := range live.GetManagedFields() {
		fields := managedFieldPaths(entry.FieldsV1)
		if len(fields) > 256 {
			fields = fields[:256]
		}
		owner := domainresource.ManagedFieldOwner{
			Manager: entry.Manager, Operation: string(entry.Operation), APIVersion: entry.APIVersion, Fields: fields,
		}
		if entry.Time != nil {
	...
	sort.SliceStable(analysis.Owners, func(i, j int) bool { return analysis.Owners[i].Manager < analysis.Owners[j].Manager })
	return analysis
}
