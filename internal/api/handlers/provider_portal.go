...
type ProviderPortalService interface {
...
type ProviderPortalHandler struct {
...
func NewProviderPortalHandler(service ProviderPortalService, identityProvider IdentityProviderService) *ProviderPortalHandler {
	return &ProviderPortalHandler{service: service, identityProvider: identityProvider}
}

func (h *ProviderPortalHandler) PortalBootstrap(c *gin.Context) {
	principal := apiMiddleware.PrincipalFromContext(c)
	item, err := h.service.PortalBootstrap(c.Request.Context(), principal)
	if err != nil {
		writeError(c, err)
		return
	}
...
func (h *ProviderPortalHandler) ListPortalApplications(c *gin.Context) {
	principal := apiMiddleware.PrincipalFromContext(c)
	items, err := h.service.ListPortalApplications(c.Request.Context(), principal)
	if err != nil {
		writeError(c, err)
		return
	}
...
func (h *ProviderPortalHandler) GetPortalApplication(c *gin.Context) {
	principal := apiMiddleware.PrincipalFromContext(c)
	item, err := h.service.GetPortalApplication(c.Request.Context(), principal, c.Param("applicationID"))
	if err != nil {
		writeError(c, err)
		return
	}
...
func (h *ProviderPortalHandler) LaunchPortalApplication(c *gin.Context) {
	principal := apiMiddleware.PrincipalFromContext(c)
	item, err := h.service.Launch(c.Request.Context(), principal, c.Param("applicationID"))
	if err != nil {
		writeError(c, err)
		return
	}
...
func (h *ProviderPortalHandler) SetFavorite(c *gin.Context) {
	principal := apiMiddleware.PrincipalFromContext(c)
	item, err := h.service.SetFavorite(c.Request.Context(), principal, c.Param("applicationID"))
	if err != nil {
		writeError(c, err)
		return
	}
...
func (h *ProviderPortalHandler) DeleteFavorite(c *gin.Context) {
	principal := apiMiddleware.PrincipalFromContext(c)
	if err := h.service.DeleteFavorite(c.Request.Context(), principal, c.Param("applicationID")); err != nil {
		writeError(c, err)
		return
	}
...
func (h *ProviderPortalHandler) ListRecent(c *gin.Context) {
	principal := apiMiddleware.PrincipalFromContext(c)
	items, err := h.service.ListRecent(c.Request.Context(), principal, parseLimit(c.Query("limit"), 10))
	if err != nil {
		writeError(c, err)
		return
	}
...
func (h *ProviderPortalHandler) SecuritySummary(c *gin.Context) {
	principal := apiMiddleware.PrincipalFromContext(c)
	apiresponse.Item(c, http.StatusOK, h.service.SecuritySummary(c.Request.Context(), principal))
}

func (h *ProviderPortalHandler) ListIdentityApplications(c *gin.Context) {
	principal := apiMiddleware.PrincipalFromContext(c)
...
func (h *ProviderPortalHandler) GetIdentityApplication(c *gin.Context) {
	principal := apiMiddleware.PrincipalFromContext(c)
...
func (h *ProviderPortalHandler) CreateIdentityApplication(c *gin.Context) {
...
func (h *ProviderPortalHandler) UpdateIdentityApplication(c *gin.Context) {
	var req domainportal.ApplicationInput
	if err := c.ShouldBindJSON(&req); err != nil {
		apiresponse.Error(c, http.StatusBadRequest, "invalid_argument", "invalid identity application payload")
		return
...
func (h *ProviderPortalHandler) DeleteIdentityApplication(c *gin.Context) {
	principal := apiMiddleware.PrincipalFromContext(c)
...
func (h *ProviderPortalHandler) ListIdentityPolicies(c *gin.Context) {
	principal := apiMiddleware.PrincipalFromContext(c)
...
func (h *ProviderPortalHandler) GetIdentityPolicy(c *gin.Context) {
	principal := apiMiddleware.PrincipalFromContext(c)
...
func (h *ProviderPortalHandler) UpdateIdentityPolicy(c *gin.Context) {
...
func (h *ProviderPortalHandler) ProviderCapabilities(c *gin.Context) {
	apiresponse.Items(c, http.StatusOK, h.service.ProviderCapabilities())
}

func (h *ProviderPortalHandler) ListIdentityProviders(c *gin.Context) {
	...
	principal := apiMiddleware.PrincipalFromContext(c)
	items, err := h.identityProvider.ListProviders(c.Request.Context(), principal, domainprovider.ProviderFilter{
...
func (h *ProviderPortalHandler) GetIdentityProvider(c *gin.Context) {
	if h.identityProvider == nil {
		writeError(c, fmt.Errorf("%w: identity provider service is not configured", apperrors.ErrUnsupportedOperation))
		return
	}
	principal := apiMiddleware.PrincipalFromContext(c)
	item, err := h.identityProvider.GetProvider(c.Request.Context(), principal, c.Param("providerID"))
	if err != nil {
		writeError(c, err)
		return
	}
...
func (h *ProviderPortalHandler) CreateIdentityProvider(c *gin.Context) {
	if h.identityProvider == nil {
		writeError(c, fmt.Errorf("%w: identity provider service is not configured", apperrors.ErrUnsupportedOperation))
		return
	}
	...
	principal := apiMiddleware.PrincipalFromContext(c)
	item, err := h.identityProvider.CreateProvider(c.Request.Context(), principal, req)
	if err != nil {
		writeError(c, err)
		return
	}
...
func (h *ProviderPortalHandler) UpdateIdentityProvider(c *gin.Context) {
	if h.identityProvider == nil {
		writeError(c, fmt.Errorf("%w: identity provider service is not configured", apperrors.ErrUnsupportedOperation))
		return
	}
	...
	principal := apiMiddleware.PrincipalFromContext(c)
	item, err := h.identityProvider.UpdateProvider(c.Request.Context(), principal, c.Param("providerID"), req)
	if err != nil {
		writeError(c, err)
		return
	}
...
func (h *ProviderPortalHandler) DeleteIdentityProvider(c *gin.Context) {
	if h.identityProvider == nil {
		writeError(c, fmt.Errorf("%w: identity provider service is not configured", apperrors.ErrUnsupportedOperation))
		return
	}
	principal := apiMiddleware.PrincipalFromContext(c)
	if err := h.identityProvider.DeleteProvider(c.Request.Context(), principal, c.Param("providerID")); err != nil {
		writeError(c, err)
		return
	}
	apiresponse.JSON(c, http.StatusOK, gin.H{"status": "ok"})
...
func (h *ProviderPortalHandler) ListOutposts(c *gin.Context) {
	if h.identityProvider == nil {
		writeError(c, fmt.Errorf("%w: identity provider service is not configured", apperrors.ErrUnsupportedOperation))
		return
	}
	principal := apiMiddleware.PrincipalFromContext(c)
	items, err := h.identityProvider.ListOutposts(c.Request.Context(), principal, domainprovider.OutpostFilter{
		Mode:   c.Query("mode"),
		Status: c.Query("status"),
		Limit:  parseLimit(c.Query("limit"), 0),
		Offset: parseOffset(c.Query("offset")),
...
func (h *ProviderPortalHandler) GetOutpost(c *gin.Context) {
	if h.identityProvider == nil {
		writeError(c, fmt.Errorf("%w: identity provider service is not configured", apperrors.ErrUnsupportedOperation))
		return
	}
	principal := apiMiddleware.PrincipalFromContext(c)
	item, err := h.identityProvider.GetOutpost(c.Request.Context(), principal, c.Param("outpostID"))
	if err != nil {
		writeError(c, err)
		return
	}
...
func (h *ProviderPortalHandler) CreateOutpost(c *gin.Context) {
	if h.identityProvider == nil {
		writeError(c, fmt.Errorf("%w: identity provider service is not configured", apperrors.ErrUnsupportedOperation))
		return
	}
	...
	principal := apiMiddleware.PrincipalFromContext(c)
	item, err := h.identityProvider.CreateOutpost(c.Request.Context(), principal, req)
	if err != nil {
		writeError(c, err)
		return
	}
...
func (h *ProviderPortalHandler) UpdateOutpost(c *gin.Context) {
	if h.identityProvider == nil {
		writeError(c, fmt.Errorf("%w: identity provider service is not configured", apperrors.ErrUnsupportedOperation))
		return
	}
	...
	principal := apiMiddleware.PrincipalFromContext(c)
	item, err := h.identityProvider.UpdateOutpost(c.Request.Context(), principal, c.Param("outpostID"), req)
	if err != nil {
		writeError(c, err)
		return
	}
...
func (h *ProviderPortalHandler) DeleteOutpost(c *gin.Context) {
	if h.identityProvider == nil {
		writeError(c, fmt.Errorf("%w: identity provider service is not configured", apperrors.ErrUnsupportedOperation))
		return
	}
	principal := apiMiddleware.PrincipalFromContext(c)
	if err := h.identityProvider.DeleteOutpost(c.Request.Context(), principal, c.Param("outpostID")); err != nil {
		writeError(c, err)
		return
	}
	apiresponse.JSON(c, http.StatusOK, gin.H{"status": "ok"})
...
func (h *ProviderPortalHandler) ClaimOutpost(c *gin.Context) {
	if h.identityProvider == nil {
		writeError(c, fmt.Errorf("%w: identity provider service is not configured", apperrors.ErrUnsupportedOperation))
		return
	}
	...
	item, err := h.identityProvider.ClaimOutpost(c.Request.Context(), req)
	if err != nil {
		writeError(c, err)
		return
	}
...
func (h *ProviderPortalHandler) HeartbeatOutpost(c *gin.Context) {
	if h.identityProvider == nil {
		writeError(c, fmt.Errorf("%w: identity provider service is not configured", apperrors.ErrUnsupportedOperation))
		return
	}
	...
	req.Token = outpostTokenFromRequest(c, req.Token)
	item, err := h.identityProvider.HeartbeatOutpost(c.Request.Context(), c.Param("outpostID"), req)
	if err != nil {
		writeError(c, err)
		return
	}
...
func (h *ProviderPortalHandler) CheckOutpost(c *gin.Context) {
	if h.identityProvider == nil {
		writeError(c, fmt.Errorf("%w: identity provider service is not configured", apperrors.ErrUnsupportedOperation))
		return
	}
	...
	item, err := h.identityProvider.CheckOutpost(c.Request.Context(), c.Param("outpostID"), req)
	if err != nil {
		writeError(c, err)
		return
	}
...
func (h *ProviderPortalHandler) OutpostEvents(c *gin.Context) {
	if h.identityProvider == nil {
		writeError(c, fmt.Errorf("%w: identity provider service is not configured", apperrors.ErrUnsupportedOperation))
		return
	}
	...
	req.Token = outpostTokenFromRequest(c, req.Token)
	item, err := h.identityProvider.RecordOutpostEvents(c.Request.Context(), c.Param("outpostID"), req)
	if err != nil {
		writeError(c, err)
		return
	}
...
func (h *ProviderPortalHandler) ListOIDCClients(c *gin.Context) {
	if h.identityProvider == nil {
		writeError(c, fmt.Errorf("%w: identity provider service is not configured", apperrors.ErrUnsupportedOperation))
		return
	}
	principal := apiMiddleware.PrincipalFromContext(c)
	items, err := h.identityProvider.ListOIDCClients(c.Request.Context(), principal, c.Param("providerID"))
	if err != nil {
		writeError(c, err)
		return
	}
...
func (h *ProviderPortalHandler) CreateOIDCClient(c *gin.Context) {
	if h.identityProvider == nil {
		writeError(c, fmt.Errorf("%w: identity provider service is not configured", apperrors.ErrUnsupportedOperation))
		return
	}
	...
	principal := apiMiddleware.PrincipalFromContext(c)
	item, err := h.identityProvider.CreateOIDCClient(c.Request.Context(), principal, c.Param("providerID"), req)
	if err != nil {
		writeError(c, err)
		return
	}
...
func (h *ProviderPortalHandler) UpdateOIDCClient(c *gin.Context) {
	if h.identityProvider == nil {
		writeError(c, fmt.Errorf("%w: identity provider service is not configured", apperrors.ErrUnsupportedOperation))
		return
	}
	...
	principal := apiMiddleware.PrincipalFromContext(c)
	item, err := h.identityProvider.UpdateOIDCClient(c.Request.Context(), principal, c.Param("clientID"), req)
	if err != nil {
		writeError(c, err)
		return
	}
...
func (h *ProviderPortalHandler) DeleteOIDCClient(c *gin.Context) {
	if h.identityProvider == nil {
		writeError(c, fmt.Errorf("%w: identity provider service is not configured", apperrors.ErrUnsupportedOperation))
		return
	}
	principal := apiMiddleware.PrincipalFromContext(c)
	if err := h.identityProvider.DeleteOIDCClient(c.Request.Context(), principal, c.Param("clientID")); err != nil {
		writeError(c, err)
		return
	}
	apiresponse.JSON(c, http.StatusOK, gin.H{"status": "ok"})
...
func (h *ProviderPortalHandler) OIDCDiscovery(c *gin.Context) {
	if h.identityProvider == nil {
		writeOIDCError(c, http.StatusServiceUnavailable, "server_error", "oidc provider service is not configured")
		return
	}
	c.JSON(http.StatusOK, h.identityProvider.Discovery(issuerFromRequest(c)))
}

func (h *ProviderPortalHandler) OIDCAuthorize(c *gin.Context) {
	if h.identityProvider == nil {
		writeOIDCError(c, http.StatusServiceUnavailable, "server_error", "oidc provider service is not configured")
		return
	}
	...
	result, err := h.identityProvider.Authorize(c.Request.Context(), issuerFromRequest(c), principal, domainprovider.AuthorizeInput{
...
func (h *ProviderPortalHandler) OIDCToken(c *gin.Context) {
	if h.identityProvider == nil {
		writeOIDCError(c, http.StatusServiceUnavailable, "server_error", "oidc provider service is not configured")
		return
	}
	clientAuth := oidcClientAuthInputFromRequest(c)
	result, err := h.identityProvider.Token(c.Request.Context(), issuerFromRequest(c), domainprovider.TokenInput{
...
func (h *ProviderPortalHandler) OIDCUserInfo(c *gin.Context) {
	if h.identityProvider == nil {
		writeOIDCError(c, http.StatusServiceUnavailable, "server_error", "oidc provider service is not configured")
		return
	}
	item, err := h.identityProvider.UserInfo(c.Request.Context(), issuerFromRequest(c), c.GetHeader("Authorization"))
	if err != nil {
...
func (h *ProviderPortalHandler) OIDCIntrospect(c *gin.Context) {
	if h.identityProvider == nil {
		writeOIDCError(c, http.StatusServiceUnavailable, "server_error", "oidc provider service is not configured")
		return
	}
	item, err := h.identityProvider.Introspect(c.Request.Context(), issuerFromRequest(c), c.PostForm("token"), oidcClientAuthInputFromRequest(c))
	if err != nil {
...
func (h *ProviderPortalHandler) OIDCRevoke(c *gin.Context) {
	if h.identityProvider == nil {
		writeOIDCError(c, http.StatusServiceUnavailable, "server_error", "oidc provider service is not configured")
		return
	}
	if err := h.identityProvider.Revoke(c.Request.Context(), issuerFromRequest(c), c.PostForm("token"), oidcClientAuthInputFromRequest(c)); err != nil {
		writeOIDCError(c, apierrors.StatusCode(err), oauthClientAuthErrorCode(err), err.Error())
		return
	}
...
func (h *ProviderPortalHandler) OIDCJWKS(c *gin.Context) {
	if h.identityProvider == nil {
		writeOIDCError(c, http.StatusServiceUnavailable, "server_error", "oidc provider service is not configured")
		return
	}
	items, err := h.identityProvider.JWKS(c.Request.Context())
	if err != nil {
...
func (h *ProviderPortalHandler) ProxyAuth(c *gin.Context) {
	if h.identityProvider == nil {
		writeProxyError(c, http.StatusServiceUnavailable, "proxy provider service is not configured")
		return
	}
	result, err := h.identityProvider.ProxyAuth(c.Request.Context(), apiMiddleware.PrincipalFromContext(c), proxyAuthInputFromRequest(c))
	if err != nil {
		writeError(c, err)
		return
	}
...
func (h *ProviderPortalHandler) ProxyStart(c *gin.Context) {
	if h.identityProvider == nil {
		writeProxyError(c, http.StatusServiceUnavailable, "proxy provider service is not configured")
		return
	}
	target := proxyReturnTo(c)
	result, err := h.identityProvider.ProxyAuth(c.Request.Context(), domainidentity.Principal{}, proxyAuthInputForReturnTo(c, target))
	if err != nil {
		writeError(c, err)
		return
	}
...
func (h *ProviderPortalHandler) ProxyCallback(c *gin.Context) {
	if h.identityProvider == nil {
		writeProxyError(c, http.StatusServiceUnavailable, "proxy provider service is not configured")
		return
	}
	...
	target := proxyReturnTo(c)
	result, err := h.identityProvider.ProxyAuth(c.Request.Context(), principal, proxyAuthInputForReturnTo(c, target))
	if err != nil {
		writeError(c, err)
		return
	}
	...
	session, err := h.identityProvider.IssueProxySession(c.Request.Context(), principal)
	if err != nil {
		writeError(c, err)
		return
	}
...
func (h *ProviderPortalHandler) ProxyLogout(c *gin.Context) {
	cookieDomain := ""
	if h.identityProvider != nil {
		if domain, err := h.identityProvider.ProxyCookieDomain(c.Request.Context(), proxyAuthInputFromRequest(c)); err == nil {
			cookieDomain = domain
		}
	}
