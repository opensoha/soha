...
func DefaultCapabilityMatrix() []CapabilityMatrixEntry {
	return []CapabilityMatrixEntry{
		...
		capability("metrics", "Metrics", "observability", scopes("cluster", "namespace"), CapabilityRiskRead, false, "/architecture/monitoring-and-alerting", available(), partial("Prometheus metrics are available when the Soha control plane can reach the configured endpoint; private in-cluster Prometheus proxying is not available")),
	}
}
