...
func DefaultCapabilityMatrix() []CapabilityMatrixEntry {
	return []CapabilityMatrixEntry{
		...
		capability("workload.mutations", "Workload mutations", "workloads", scopes("cluster", "namespace"), CapabilityRiskMutate, true, "/architecture/authorization", available(), partial("deployment restart, rollback, and scale are available through the agent; pod deletion and YAML apply remain direct-only")),
		...
		capability("resource.yaml.apply", "YAML apply and delete", "configuration", scopes("cluster", "namespace"), CapabilityRiskMutate, true, "/operations/agent-runtime", available(), partial("built-in and custom-resource YAML apply/delete are available through the agent; built-in create flows remain direct-only")),
		capability("configuration.inventory", "Configuration inventory", "configuration", scopes("cluster", "namespace"), CapabilityRiskRead, false, "/architecture/multi-cluster-model", available(), partial("ConfigMap and Secret lists are available through the agent; detail views and create flows remain direct-only")),
		capability("rbac.inventory", "RBAC inventory", "access", scopes("cluster", "namespace"), CapabilityRiskRead, false, "/architecture/access-model", available(), partial("RBAC lists and several detail views are available through the agent; create flows remain direct-only")),
		...
		capability("storage.inventory", "Storage inventory", "storage", scopes("cluster", "namespace"), CapabilityRiskRead, false, "/architecture/multi-cluster-model", available(), partial("storage lists are available through the agent; PVC, PV, and StorageClass detail and create flows remain direct-only")),
