...
var defaultToolCatalog = []domainaigateway.ToolCapability{
	{
		...
		PermissionKeys: []string{appaccess.PermAIGatewayInvoke, appaccess.PermNetworkAccessMihomoProfilesView},
		RequiredScopes: []string{"device"},
		MCPAdapterID:   "network-access.v1",
		MCPToolName:    "network_access.mihomo_profiles.list",
		InputSchema: gatewayObjectSchema(nil, map[string]any{
			"search": gatewayStringSchema("Optional profile search text."), "deviceId": gatewayStringSchema("Optional endpoint device id."),
			"mode":   map[string]any{"type": "string", "enum": []any{"managed_follow", "app_subscription"}},
	...
	{
		Name:           "network_access.telemetry.summary",
		Title:          "Summarize Network Telemetry",
		Description:    "Read bounded heartbeat, RADIUS accounting, network flow, and mihomo aggregate telemetry without raw destinations or event payloads.",
		Domain:         "network_access",
		Action:         "summary",
		RiskLevel:      domainaigateway.RiskLevelRead,
		PermissionKeys: []string{appaccess.PermAIGatewayInvoke, appaccess.PermNetworkAccessTelemetryView},
		RequiredScopes: []string{"producer"},
		MCPAdapterID:   "network-access.v1",
		MCPToolName:    "network_access.telemetry.summary",
		InputSchema: gatewayObjectSchema(nil, map[string]any{
			"from": map[string]any{"type": "string", "format": "date-time"}, "to": map[string]any{"type": "string", "format": "date-time"},
			"producerId": gatewayStringSchema("Optional telemetry producer id."),
			"limit":      map[string]any{"type": "integer", "minimum": 1, "maximum": 200, "default": 100},
		}),
	},
	{
		Name:           "network_access.conflicts.analyze",
		Title:          "Analyze Network Address Conflicts",
		Description:    "Compare persisted network spaces with LAN, WireGuard, container, Kubernetes, reserved, and mihomo fake-IP ranges.",
		Domain:         "network_access",
		Action:         "analyze",
		RiskLevel:      domainaigateway.RiskLevelAnalyze,
		PermissionKeys: []string{appaccess.PermAIGatewayInvoke, appaccess.PermNetworkAccessPolicyView},
		RequiredScopes: []string{"policy"},
		MCPAdapterID:   "network-access.v1",
		MCPToolName:    "network_access.conflicts.analyze",
		InputSchema:    networkConflictInputSchema(),
		OutputSchema:   networkConflictOutputSchema(),
	},
	{
		Name:             "network_access.access_grants.create",
		Title:            "Create Network Resource Access Grant",
		Description:      "Create an MFA-bound, short-lived, one-time grant for WireGuard ZTNA resource access.",
		Domain:           "network_access",
		Action:           "create",
		RiskLevel:        domainaigateway.RiskLevelHigh,
		PermissionKeys:   []string{appaccess.PermAIGatewayInvoke, appaccess.PermNetworkAccessAccessGrantsCreate},
		RequiredScopes:   []string{"subject", "device", "site", "resource"},
		MCPAdapterID:     "network-access.v1",
		MCPToolName:      "network_access.access_grants.create",
		RequiresApproval: true,
		InputSchema:      networkAccessGrantCreateInputSchema(),
		OutputSchema:     networkAccessGrantSecretSchema(),
	},
	{
		Name:           "network_access.access_grants.list",
		Title:          "List Network Resource Access Grants",
		Description:    "List short-lived resource access grants without token material or token digests.",
		Domain:         "network_access",
		Action:         "list",
		RiskLevel:      domainaigateway.RiskLevelRead,
		PermissionKeys: []string{appaccess.PermAIGatewayInvoke, appaccess.PermNetworkAccessAccessGrantsView},
		RequiredScopes: []string{"grant"},
		MCPAdapterID:   "network-access.v1",
		MCPToolName:    "network_access.access_grants.list",
		InputSchema:    networkAccessGrantFilterSchema(),
		OutputSchema:   networkAccessGrantListSchema(),
	},
	{
		Name:             "network_access.access_grants.revoke",
		Title:            "Revoke Network Resource Access Grant",
		Description:      "Revoke an unconsumed network resource access grant.",
		Domain:           "network_access",
		Action:           "revoke",
		RiskLevel:        domainaigateway.RiskLevelHigh,
		PermissionKeys:   []string{appaccess.PermAIGatewayInvoke, appaccess.PermNetworkAccessAccessGrantsRevoke},
		RequiredScopes:   []string{"grant"},
		MCPAdapterID:     "network-access.v1",
		MCPToolName:      "network_access.access_grants.revoke",
		RequiresApproval: true,
		InputSchema: gatewayObjectSchema([]string{"grantId"}, map[string]any{
			"grantId": gatewayStringSchema("Access grant id to revoke."),
		}),
		OutputSchema: networkAccessGrantSchema(),
	},
	{
		Name:             "network_access.gateways.create",
		Title:            "Create Network Gateway",
		Description:      "Create an independently operated WireGuard gateway definition without placing user traffic on the Soha control plane.",
		Domain:           "network_access",
		Action:           "create",
		RiskLevel:        domainaigateway.RiskLevelHigh,
		PermissionKeys:   []string{appaccess.PermAIGatewayInvoke, appaccess.PermNetworkAccessGatewaysCreate},
		RequiredScopes:   []string{"site", "runtime"},
		MCPAdapterID:     "network-access.v1",
		MCPToolName:      "network_access.gateways.create",
		RequiresApproval: true,
		InputSchema:      networkGatewayInputSchema(false),
		OutputSchema:     networkGatewaySchema(),
	},
	{
		Name:             "network_access.gateways.update",
		Title:            "Update Network Gateway",
		Description:      "Update an existing WireGuard gateway definition, including its split-tunnel overlay and routing mode.",
		Domain:           "network_access",
		Action:           "update",
		RiskLevel:        domainaigateway.RiskLevelHigh,
		PermissionKeys:   []string{appaccess.PermAIGatewayInvoke, appaccess.PermNetworkAccessGatewaysUpdate},
		RequiredScopes:   []string{"gateway", "site", "runtime"},
		MCPAdapterID:     "network-access.v1",
		MCPToolName:      "network_access.gateways.update",
		RequiresApproval: true,
		InputSchema:      networkGatewayInputSchema(true),
		OutputSchema:     networkGatewaySchema(),
	},
	{
		Name:             "network_access.enrollments.create",
		Title:            "Create Network Runtime Enrollment",
		Description:      "Create a short-lived one-time enrollment token for an endpoint, gateway, or NAS runtime.",
		Domain:           "network_access",
		Action:           "create",
		RiskLevel:        domainaigateway.RiskLevelHigh,
		PermissionKeys:   []string{appaccess.PermAIGatewayInvoke, appaccess.PermNetworkAccessEnrollmentsCreate},
		RequiredScopes:   []string{"subject", "device", "runtime"},
		MCPAdapterID:     "network-access.v1",
		MCPToolName:      "network_access.enrollments.create",
		RequiresApproval: true,
		InputSchema:      networkEnrollmentCreateInputSchema(),
		OutputSchema:     networkEnrollmentSecretSchema(),
	},
	{
		Name:           "network_access.enrollments.list",
		Title:          "List Network Runtime Enrollments",
		Description:    "List runtime enrollment challenges without token material or token digests.",
		Domain:         "network_access",
		Action:         "list",
		RiskLevel:      domainaigateway.RiskLevelRead,
		PermissionKeys: []string{appaccess.PermAIGatewayInvoke, appaccess.PermNetworkAccessEnrollmentsView},
		RequiredScopes: []string{"enrollment"},
		MCPAdapterID:   "network-access.v1",
		MCPToolName:    "network_access.enrollments.list",
		InputSchema:    networkEnrollmentListInputSchema(),
		OutputSchema:   networkEnrollmentListSchema(),
	},
	{
		Name:             "network_access.enrollments.revoke",
		Title:            "Revoke Network Runtime Enrollment",
		Description:      "Revoke a pending network runtime enrollment challenge.",
		Domain:           "network_access",
		Action:           "revoke",
		RiskLevel:        domainaigateway.RiskLevelHigh,
		PermissionKeys:   []string{appaccess.PermAIGatewayInvoke, appaccess.PermNetworkAccessEnrollmentsRevoke},
		RequiredScopes:   []string{"enrollment"},
		MCPAdapterID:     "network-access.v1",
		MCPToolName:      "network_access.enrollments.revoke",
		RequiresApproval: true,
		InputSchema: gatewayObjectSchema([]string{"enrollmentId"}, map[string]any{
			"enrollmentId": gatewayStringSchema("Enrollment id to revoke."),
		}),
		OutputSchema: networkEnrollmentSchema(),
	},
	{
		Name:           "network_access.policy.preview",
		Title:          "Preview Network Access Policy",
		Description:    "Evaluate a user, device, resource, site, and access mode without creating a network or resource lease.",
		Domain:         "network_access",
		Action:         "preview",
		RiskLevel:      domainaigateway.RiskLevelAnalyze,
		PermissionKeys: []string{appaccess.PermAIGatewayInvoke, appaccess.PermNetworkAccessPolicyView},
		RequiredScopes: []string{"subject", "policy"},
		MCPAdapterID:   "network-access.v1",
		MCPToolName:    "network_access.policy.preview",
		InputSchema: gatewayObjectSchema([]string{"subjectUserId", "deviceId", "resourceId", "mode"}, map[string]any{
			"subjectUserId": gatewayStringSchema("User id to evaluate."),
			"deviceId":      gatewayStringSchema("Endpoint device id to evaluate."),
			"resourceId":    gatewayStringSchema("Network resource id to evaluate."),
			"siteId":        gatewayStringSchema("Optional asserted site id."),
			"mode": map[string]any{
				"type": "string",
				"enum": []any{"internal_direct", "internal_ztna", "external_vpn", "external_vpn_ztna", "external_direct_ztna"},
			},
		}),
		OutputSchema: networkPolicyPreviewOutputSchema(),
	},
	{
		Name:           "network_access.sessions.list",
		Title:          "List Network Sessions",
		Description:    "List active and historical network sessions by site, NAS runtime, subject, device, or status.",
		Domain:         "network_access",
		Action:         "list",
		RiskLevel:      domainaigateway.RiskLevelRead,
		PermissionKeys: []string{appaccess.PermAIGatewayInvoke, appaccess.PermNetworkAccessSitesView},
		RequiredScopes: []string{"session"},
		MCPAdapterID:   "network-access.v1",
		MCPToolName:    "network_access.sessions.list",
		InputSchema:    networkSessionFilterSchema(),
		OutputSchema:   networkSessionListSchema(),
	},
	{
		Name:           "network_access.session.coa.plan",
		Title:          "Plan Network Session Enforcement",
		Description:    "Preview a CoA or disconnect action, including capability-based fallback, without changing the session.",
		Domain:         "network_access",
		Action:         "plan",
		RiskLevel:      domainaigateway.RiskLevelAnalyze,
		PermissionKeys: []string{appaccess.PermAIGatewayInvoke, appaccess.PermNetworkAccessSitesUpdate},
		RequiredScopes: []string{"session"},
		MCPAdapterID:   "network-access.v1",
		MCPToolName:    "network_access.session.coa.plan",
		InputSchema:    networkSessionActionInputSchema(false),
		OutputSchema:   networkSessionActionPlanSchema(),
	},
	{
		Name:             "network_access.session.coa.execute",
		Title:            "Execute Network Session Enforcement",
		Description:      "Queue the exact approved CoA or disconnect plan for its owning NAS runtime.",
		Domain:           "network_access",
		Action:           "execute",
		RiskLevel:        domainaigateway.RiskLevelHigh,
		PermissionKeys:   []string{appaccess.PermAIGatewayInvoke, appaccess.PermNetworkAccessSitesUpdate},
		RequiredScopes:   []string{"session"},
		MCPAdapterID:     "network-access.v1",
		MCPToolName:      "network_access.session.coa.execute",
		RequiresApproval: true,
		InputSchema:      networkSessionActionInputSchema(true),
		OutputSchema:     networkSessionCommandSchema(),
	},
	{
	...
	},
	{
		...
		Description:    "List namespace metadata from the scoped Kubernetes workbench API.",
		Domain:         "k8s",
		Action:         "list",
		RiskLevel:      domainaigateway.RiskLevelRead,
		PermissionKeys: []string{appaccess.PermAIGatewayInvoke, appaccess.PermWorkspaceResourceView, appaccess.PermPlatformNamespacesView},
		RequiredScopes: []string{"cluster", "namespace"},
		MCPAdapterID:   "platform-native.v1",
		MCPToolName:    "k8s.namespaces.list",
		InputSchema:    gatewayObjectSchema([]string{"clusterId"}, gatewayClusterNamespaceProperties("Optional namespace scope used by the gateway authorization policy.")),
	},
	{
		Name:           "k8s.workloads.overview",
		Title:          "Read Workload Overview",
		Description:    "Read pod health totals, namespace breakdown, and bounded problematic pod summaries.",
		Domain:         "k8s",
		Action:         "read",
		RiskLevel:      domainaigateway.RiskLevelRead,
		PermissionKeys: []string{appaccess.PermAIGatewayInvoke, appaccess.PermWorkspaceResourceView, appaccess.PermPlatformWorkloadsOverviewView},
		RequiredScopes: []string{"cluster", "namespace"},
		MCPAdapterID:   "platform-native.v1",
		MCPToolName:    "k8s.workloads.overview",
		InputSchema:    gatewayObjectSchema([]string{"clusterId"}, gatewayClusterNamespaceProperties("Workload namespace. Empty means all namespaces when allowed.")),
	},
	{
		Name:           "k8s.configmaps.list",
		Title:          "List ConfigMaps",
		Description:    "List ConfigMap metadata and data entry counts without returning ConfigMap content.",
		Domain:         "k8s",
		Action:         "list",
		RiskLevel:      domainaigateway.RiskLevelRead,
		PermissionKeys: []string{appaccess.PermAIGatewayInvoke, appaccess.PermWorkspaceResourceView, appaccess.PlatformActionPermission("configuration", "ConfigMap", "view")},
		RequiredScopes: []string{"cluster", "namespace"},
		MCPAdapterID:   "platform-native.v1",
		MCPToolName:    "k8s.configmaps.list",
		InputSchema:    gatewayObjectSchema([]string{"clusterId"}, gatewayClusterNamespaceProperties("ConfigMap namespace. Empty means all namespaces when allowed.")),
	},
	{
		Name:           "k8s.secrets.metadata",
		Title:          "List Secret Metadata",
		Description:    "List Secret names, types, and data entry counts without returning Secret data.",
		Domain:         "k8s",
		Action:         "list",
		RiskLevel:      domainaigateway.RiskLevelRead,
		PermissionKeys: []string{appaccess.PermAIGatewayInvoke, appaccess.PermWorkspaceResourceView, appaccess.PlatformActionPermission("configuration", "Secret", "view")},
		RequiredScopes: []string{"cluster", "namespace"},
		MCPAdapterID:   "platform-native.v1",
		MCPToolName:    "k8s.secrets.metadata",
		InputSchema:    gatewayObjectSchema([]string{"clusterId"}, gatewayClusterNamespaceProperties("Secret namespace. Empty means all namespaces when allowed.")),
	},
	{
		Name:           "k8s.helm.releases.list",
		Title:          "List Helm Releases",
		Description:    "List Helm release metadata without returning release values or rendered manifests.",
		Domain:         "k8s",
		Action:         "list",
		RiskLevel:      domainaigateway.RiskLevelRead,
		PermissionKeys: []string{appaccess.PermAIGatewayInvoke, appaccess.PermWorkspaceResourceView, appaccess.PermPlatformHelmView},
		RequiredScopes: []string{"cluster", "namespace"},
		MCPAdapterID:   "platform-native.v1",
		MCPToolName:    "k8s.helm.releases.list",
	...
	},
	{
		...
		Action:         "logs",
		RiskLevel:      domainaigateway.RiskLevelRead,
		PermissionKeys: []string{appaccess.PermAIGatewayInvoke, appaccess.PermWorkspaceResourceView, appaccess.PermPlatformPodsLogs},
	...
	},
	{
		...
		Action:         "list",
		RiskLevel:      domainaigateway.RiskLevelRead,
		PermissionKeys: []string{appaccess.PermAIGatewayInvoke, appaccess.PermWorkspaceResourceView, appaccess.PermObserveEventsView, appaccess.PermPlatformWorkloadsOverviewView},
	...
	},
	{
		...
		Action:         "list",
		RiskLevel:      domainaigateway.RiskLevelRead,
		PermissionKeys: []string{appaccess.PermAIGatewayInvoke, appaccess.PermWorkspaceResourceView, appaccess.PlatformActionPermission("network", "Service", "view")},
		RequiredScopes: []string{"cluster", "namespace"},
		MCPAdapterID:   "platform-native.v1",
	...
	},
	{
		...
		Action:         "read",
		RiskLevel:      domainaigateway.RiskLevelRead,
		PermissionKeys: []string{appaccess.PermAIGatewayInvoke, appaccess.PermWorkspaceResourceView, appaccess.PlatformActionPermission("network", "Service", "view"), appaccess.PermPlatformPodsView, appaccess.PlatformActionPermission("network", "Ingress", "view")},
	...
	},
	{
		...
		PermissionKeys: []string{
			...
			appaccess.PlatformActionPermission("network", "Ingress", "view"),
			appaccess.PlatformActionPermission("network", "GatewayClass", "view"),
			appaccess.PlatformActionPermission("network", "Gateway", "view"),
			appaccess.PlatformActionPermission("network", "HTTPRoute", "view"),
			appaccess.PlatformActionPermission("network", "BackendTLSPolicy", "view"),
			appaccess.PlatformActionPermission("network", "GRPCRoute", "view"),
			appaccess.PlatformActionPermission("network", "ReferenceGrant", "view"),
		},
		RequiredScopes: []string{"cluster", "namespace", "service"},
		MCPAdapterID:   "platform-native.v1",
	...
	},
	{
		...
		PermissionKeys: []string{
			appaccess.PermAIGatewayInvoke,
			appaccess.PermWorkspaceResourceView,
			appaccess.PlatformActionPermission("storage", "PersistentVolumeClaim", "view"),
			appaccess.PlatformActionPermission("storage", "PersistentVolume", "view"),
			appaccess.PlatformActionPermission("storage", "StorageClass", "view"),
		},
	...
	},
	{
		...
		Action:         "list",
		RiskLevel:      domainaigateway.RiskLevelRead,
		PermissionKeys: []string{appaccess.PermAIGatewayInvoke, appaccess.ManagedActionPermission(appaccess.PermAIGatewayClientsManage, "view")},
	...
	},
	{
		...
		Action:         "list",
		RiskLevel:      domainaigateway.RiskLevelRead,
		PermissionKeys: []string{appaccess.PermAIGatewayInvoke, appaccess.ManagedActionPermission(appaccess.PermAIGatewayTokensManage, "view")},
	...
	},
	{
		...
		Action:         "list",
		RiskLevel:      domainaigateway.RiskLevelRead,
		PermissionKeys: []string{appaccess.PermAIGatewayInvoke, appaccess.ManagedActionPermission(appaccess.PermAIGatewayGrantsManage, "view")},
	...
	},
	{
		...
		Action:         "list",
		RiskLevel:      domainaigateway.RiskLevelRead,
		PermissionKeys: []string{appaccess.PermAIGatewayInvoke, appaccess.ManagedActionPermission(appaccess.PermAIGatewayPoliciesManage, "view")},
	...
	},
	{
		...
		Action:         "list",
		RiskLevel:      domainaigateway.RiskLevelRead,
		PermissionKeys: []string{appaccess.PermAIGatewayInvoke, appaccess.ManagedActionPermission(appaccess.PermAIGatewaySkillsManage, "view")},
		RequiredScopes: []string{"aiClient", "skill"},
		MCPAdapterID:   "gateway-governance.v1",
	...
	},
	{
		...
		Action:         "list",
		RiskLevel:      domainaigateway.RiskLevelRead,
		PermissionKeys: []string{appaccess.PermAIGatewayInvoke, appaccess.ManagedActionPermission(appaccess.PermAIGatewayApprovalsManage, "view")},
	...
	},
	{
		...
		Action:           "execute",
		RiskLevel:        domainaigateway.RiskLevelExecute,
		PermissionKeys:   []string{appaccess.PermAIGatewayInvoke, appaccess.ManagedActionPermission(appaccess.PermAIGatewayRelayManage, "delete")},
...
var operationsToolCatalog = []domainaigateway.ToolCapability{
	{
		Name: "compute.overview.read", Title: "Read Compute Overview", Description: "Read the permission-filtered virtualization, runtime, task, and provider health overview.",
		Domain: "compute", Action: "read", RiskLevel: domainaigateway.RiskLevelRead,
		PermissionKeys: []string{appaccess.PermAIGatewayInvoke}, RequiredScopes: []string{"computeDomain"}, MCPAdapterID: "compute.v1", MCPToolName: "compute.overview.read",
		InputSchema: gatewayObjectSchema(nil, map[string]any{}),
	},
	{
		Name: "compute.resources.read", Title: "Read Compute Resource", Description: "Read one normalized compute resource through the unified provider surface.",
		Domain: "compute", Action: "read", RiskLevel: domainaigateway.RiskLevelRead,
		PermissionKeys: []string{appaccess.PermAIGatewayInvoke}, RequiredScopes: []string{"computeDomain", "computeResource"}, MCPAdapterID: "compute.v1", MCPToolName: "compute.resources.read",
		InputSchema: gatewayObjectSchema([]string{"domain", "kind", "id"}, map[string]any{
			"domain": gatewayStringSchema("Compute domain: virtualization, container_runtime, or agent."),
			"kind":   gatewayStringSchema("Normalized compute resource kind."), "id": gatewayStringSchema("Resource id."),
		}),
	},
	{
		Name: "compute.resource_relations.list", Title: "List Compute Resource Relations", Description: "List normalized relationships around one compute resource.",
		Domain: "compute", Action: "list", RiskLevel: domainaigateway.RiskLevelRead,
		PermissionKeys: []string{appaccess.PermAIGatewayInvoke}, RequiredScopes: []string{"computeDomain", "computeResource"}, MCPAdapterID: "compute.v1", MCPToolName: "compute.resource_relations.list",
		InputSchema: gatewayObjectSchema([]string{"domain", "kind", "id"}, map[string]any{
			"domain": gatewayStringSchema("Compute domain."), "kind": gatewayStringSchema("Normalized compute resource kind."),
			"id": gatewayStringSchema("Resource id."), "cursor": gatewayStringSchema("Optional relation cursor."), "limit": gatewayIntegerSchema("Maximum relations to return."),
		}),
	},
	{
		Name: "compute.tasks.list", Title: "List Compute Tasks", Description: "List permission-filtered virtualization and container runtime tasks.",
		Domain: "compute", Action: "list", RiskLevel: domainaigateway.RiskLevelRead,
		PermissionKeys: []string{appaccess.PermAIGatewayInvoke}, RequiredScopes: []string{"computeDomain", "computeTask"}, MCPAdapterID: "compute.v1", MCPToolName: "compute.tasks.list",
		InputSchema: gatewayObjectSchema(nil, map[string]any{
			"domain": gatewayStringSchema("Optional task domain."), "providerKey": gatewayStringSchema("Optional provider key."),
			"status": gatewayStringSchema("Optional normalized task status."), "category": gatewayStringSchema("Optional task category."),
			"resourceKind": gatewayStringSchema("Optional exact resource kind."), "resourceId": gatewayStringSchema("Optional exact resource id."),
			"sortBy": gatewayStringSchema("Optional sort field."), "sortOrder": gatewayStringSchema("Optional asc or desc order."),
			"cursor": gatewayStringSchema("Optional task cursor."), "limit": gatewayIntegerSchema("Maximum tasks to return."),
		}),
	},
	{
		Name: "compute.tasks.get", Title: "Read Compute Task", Description: "Read one normalized compute task and its verification state.",
		Domain: "compute", Action: "read", RiskLevel: domainaigateway.RiskLevelRead,
		PermissionKeys: []string{appaccess.PermAIGatewayInvoke}, RequiredScopes: []string{"computeDomain", "computeTask"}, MCPAdapterID: "compute.v1", MCPToolName: "compute.tasks.get",
		InputSchema: gatewayObjectSchema([]string{"domain", "taskId"}, map[string]any{
			"domain": gatewayStringSchema("Task domain."), "taskId": gatewayStringSchema("Task id."),
		}),
	},
	{
		Name: "compute.task_logs.list", Title: "List Compute Task Logs", Description: "Read redacted logs for one compute task.",
		Domain: "compute", Action: "logs", RiskLevel: domainaigateway.RiskLevelRead,
		PermissionKeys: []string{appaccess.PermAIGatewayInvoke}, RequiredScopes: []string{"computeDomain", "computeTask"}, MCPAdapterID: "compute.v1", MCPToolName: "compute.task_logs.list",
		InputSchema: gatewayObjectSchema([]string{"domain", "taskId"}, map[string]any{
			"domain": gatewayStringSchema("Task domain."), "taskId": gatewayStringSchema("Task id."),
		}),
	},
	...
	},
	{
		...
		PermissionKeys: []string{appaccess.PermAIGatewayInvoke, appaccess.PermVirtualizationVMsCreate}, RequiredScopes: []string{"virtualizationConnection", "vm"},
	...
	},
	{
		...
		Domain: "virtualization", Action: "execute", RiskLevel: domainaigateway.RiskLevelExecute, RequiresApproval: true,
		PermissionKeys: []string{appaccess.PermAIGatewayInvoke}, RequiredScopes: []string{"virtualizationConnection", "vm"},
...
func defaultSkills() []domainaigateway.SkillCapability {
	return []domainaigateway.SkillCapability{
		...
		},
		{
			...
			CapabilityRefs: []string{"k8s.namespaces.list", "k8s.workloads.overview", "k8s.configmaps.list", "k8s.secrets.metadata", "k8s.helm.releases.list", "k8s.pods.list", "k8s.pods.logs", "k8s.pods.describe", "k8s.deployments.list", "k8s.deployments.rollout_status", "k8s.deployments.events", "k8s.services.list", "k8s.services.backends", "k8s.routes.context", "k8s.storage.context", "k8s.nodes.detail", "k8s.events.list"},
			PermissionKeys: []string{appaccess.PermAIGatewayInvoke, appaccess.PermWorkspaceResourceView},
			RequiredScopes: []string{"cluster", "namespace"},
		},
		{
			...
			Name:           "Compute SRE",
			Category:       "platform",
			Description:    "Read-only compute overview, resource relation, task, and task-log diagnosis.",
			CapabilityRefs: []string{"compute.overview.read", "compute.resources.read", "compute.resource_relations.list", "compute.tasks.list", "compute.tasks.get", "compute.task_logs.list"},
			PermissionKeys: []string{appaccess.PermAIGatewayInvoke},
			RequiredScopes: []string{"computeDomain", "computeResource", "computeTask"},
		},
		{
