...
该命令会在 `dist/helm-repo/` 生成 `soha`、`soha-agent`、`soha-hermes-agent` 三个 chart 的 `index.yaml` 和 chart tarball。把该目录发布到独立 `opensoha/soha-helm` 仓库的 GitHub Pages，例如其 `gh-pages` 分支根目录。Artifact Hub 不托管 chart 包，它索引这个 Helm repo URL；在 Artifact Hub 添加 Helm repository 时填写 `https://opensoha.github.io/soha-helm`。需要认领仓库时，把 [deploy/chart/artifacthub-repo.yml.example](./deploy/chart/artifacthub-repo.yml.example) 复制为 `deploy/chart/artifacthub-repo.yml`，填入维护者邮箱后重新生成并发布 Helm repo。
