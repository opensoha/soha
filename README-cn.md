...
├── deploy/              # 镜像构建、compose、k8s 与 Helm 部署资产
...
└── docker-compose.yml   # 轻量级本地 PostgreSQL 启动文件
```
...
根目录的 `docker-compose.yml` 仅用于本地开发数据库启动，保持最小化。
...
默认后端配置文件为 [configs/config.yaml](./configs/config.yaml)。如需覆盖，使用 `KC_CONFIG_FILE=/abs/path/to/config.yaml`。
...
仓库正式的部署资产目录位于 [deploy](./deploy/)。

- [deploy/docker/Dockerfile.single-project](./deploy/docker/Dockerfile.single-project): 单项目镜像构建入口，内嵌 API、SPA 与 docs
- [deploy/compose/docker-compose.single-project.yml](./deploy/compose/docker-compose.single-project.yml): 本地完整栈启动文件
...
- [deploy/config/config.api.single-project.yaml](./deploy/config/config.api.single-project.yaml): 单项目部署用的应用配置模板
