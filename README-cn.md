...
soha 是一个完整的全栈平台控制台项目，后端基于 Go，前端基于 React + Ant Design，文档站点基于 Docusaurus 并随仓库一起维护。它的目标不只是资源浏览器，而是逐步演进为统一的多集群控制平面。
...
- 可插拔 AI Agent Runtime 以 Hermes 作为首个外部 provider，同时由 soha 统一控制 capability、tool binding、skills、预算、审计和 `AnalysisArtifact` 输出
...
└── docker-compose.yaml  # soha 与 PostgreSQL 的本地 compose 入口
```
...
默认 `make` 会同时启动 Go API 与 Vite 前端。后端默认读取 [configs/config.yaml](./configs/config.yaml)，如需覆盖，使用 `SOHA_CONFIG_FILE=/abs/path/to/config.yaml`。
...
默认 agent 配置文件为 [configs/agent.config.yaml](./configs/agent.config.yaml)。如需覆盖，使用 `SOHA_AGENT_CONFIG_FILE=/abs/path/to/agent.config.yaml`。
...
- 平台 API 返回 soha 聚合 DTO，不直接返回原始 Kubernetes 对象，YAML 或明确透传接口除外。空 namespace 表示 namespaced 资源的全命名空间聚合；集群级资源应忽略 namespace 过滤。
...
- AI Agent Runtime 必须让页面、自动化策略和业务模块只依赖 soha provider/capability/tool/skill 合约。Hermes 只是 claim/callback API 背后的 provider runner，agent 输出必须转换回 `AnalysisArtifact`。
...
soha 目前处于持续开发中。平台管理、交付、可观测性与 AI 模块仍在持续收敛，不同模块成熟度并不完全一致。当前工程基线与范围决策以 [AGENTS.md](./AGENTS.md) 为准。
